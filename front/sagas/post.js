...
function removePostAPI(postId, postBoardNum) {
  return axios.delete(
    `/post/delete?postId=${postId}&postBoardNum=${postBoardNum}`
  ); // delete는 데이터를 가져갈 수 없다 data는 postId
}
...
function updateSPostAPI(postId, postBoardNum, postStatus) {
  return axios.get(
    `/post/status?postId=${postId}&postBoardNum=${postBoardNum}&postStatus=${postStatus}`
  );
}
